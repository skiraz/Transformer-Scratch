...
class Attention_Block(nn.Module):
    ...
    def forward(self,embedding=None,Q=None,K=None,V=None):
        ...
        #scale 
        self.att_scores = self.att_scores/ (key_dim**(1/2))
        

        #mask
        if self.mask:
            trill = torch.tril(torch.ones((batch,self.heads ,query_T,query_T ))).to("cuda")
            self.att_scores = self.att_scores.masked_fill(trill==0,float("-inf")).to("cuda")
            del trill
        ...
        #softmax  
        self.att_scores = F.softmax(self.att_scores,dim=2)
